...
def build_brochure(analysis_workbook: Path | None, template: Path, output_folder: Path, trial_name: str = ""):
    ...
    # COM builder duplicates its first slide and replaces matching text/image slots.
    script = Path(__file__).with_name("build_brochure.ps1")
    env = os.environ.copy(); env["BROCHURE_MANIFEST"] = str(manifest_path); env["BROCHURE_TEMPLATE"] = str(template); env["BROCHURE_OUTPUT"] = str(run_folder / "Variety Brochure.pptx"); env["BROCHURE_ANALYSIS"] = str(analysis_workbook or "")
    import subprocess
    subprocess.run(["powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(script)], env=env, check=True)
    manifest_path.unlink(missing_ok=True)
    return data_path, Path(env["BROCHURE_OUTPUT"])
